...
class WuiwidgetComponent extends ApplicationComponent
{
    ...
    function DoInstallAction ($params)
    {
        ...
        if (strlen($params['file'])) {
            $params['file'] = $this->basedir . '/WEB-INF/classes/shared/wui/' . basename($params['file']);
            if (@copy($params['file'], InnomaticContainer::instance('innomaticcontainer')->getHome() . 'WEB-INF/classes/shared/wui/' . basename($params['file']))) {
                @chmod(InnomaticContainer::instance('innomaticcontainer')->getHome() . 'WEB-INF/classes/shared/wui/' . basename($params['file']), 0644);
    ...
    function DoUninstallAction ($params)
    {
        ...
        if (strlen($params['file'])) {
            if ($this->rootda->execute('DELETE FROM wui_widgets WHERE name=' . $this->rootda->formatText($params['name']))) {
                if (@unlink(InnomaticContainer::instance('innomaticcontainer')->getHome() . 'WEB-INF/classes/shared/wui/' . basename($params['file']))) {
                    $result = true;
                }
            } else
