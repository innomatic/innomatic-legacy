...
class FullNestedSet extends BaseNestedSet implements ExtendedNestedSet
{
     ...
     *            the SQL condition, such as Title=?
     * @param string $Rest
     *            optional, rest of variables to fill in placeholders of condition string, one variable for each ? in condition
     * @return Integer ID
     */
    function GetID($ConditionString, $Rest = null)
    {
        // TODO finish (Alex Pagnoni)
        $args = func_get_args();
        array_shift($args);
    ...
    function DepthConditional($ConditionString, $Rest = null)
    {
        ...
        $Path = call_user_func_array(array(
            $this,
            "PathConditional"
        ), $Arguments);
     ...
     *            from current node (negative or positive)
     * @param string $ConditionString            
     * @param string $Rest
     *            optional, rest of variables to fill in placeholders of condition string, one variable for each ? in condition
    ...
    function SiblingConditional($SiblingDistance = 1, $ConditionString, $Rest = null)
    {
        ...
        $Parent = call_user_func_array(array(
            $this,
    ...
    function DeleteConditional($ConditionString, $Rest = null)
    {
        ...
        jf::SQL("UPDATE {$this->Table()} SET {$this->Right()} = {$this->Right()} - 1, {$this->Left()} = {$this->Left()} - 1 WHERE {$this->Left()} BETWEEN {$Info["Left"]} AND {$Info["Right"]}");
     ...
     *            to return Depth of sub-tree from zero or absolutely from the whole tree
     * @param string $Condition            
     * @param string $Rest
     *            optional, rest of variables to fill in placeholders of condition string, one variable for each ? in condition
     ...
     *            Pairs of Key/Value as Field/Value in the table
     * @param string $ConditionString            
     * @param string $Rest
     *            optional, rest of variables to fill in placeholders of condition string
     ...
     *
     * @param Array $FieldValueArray
     *            key-paired field-values to insert
     * @param string $ConditionString
     *            of the parent node
     * @param string $Rest
     *            optional, rest of variables to fill in placeholders of condition string, one variable for each ? in condition
     ...
     *
     * @param Array $FieldValueArray
     *            Pairs of Key/Value as Field/Value in the table to edit
     * @param string $ConditionString            
     * @param string $Rest
     *            optional, rest of variables to fill in placeholders of condition string, one variable for each ? in condition
     * @return Integer SiblingID
