...
class PermissionManager extends Base
{
    ...
    {
        $this->domain_permissions = new \Innomatic\Dataaccess\Nestedset\FullNestedSet ( "domain_permissions", "id", "left", "right" );
    }
    /**
    ...
    function Roles($Permission, $OnlyIDs = true)
    {
        ...
        if ($OnlyIDs)
        {
            $Res = jf::SQL ( "SELECT roleid AS id FROM
				domain_roles_permissions WHERE permissionid=? ORDER BY roleid", $Permission );
            if (is_array ( $Res ))
            {
                $out = array ();
                foreach ( $Res as $R )
