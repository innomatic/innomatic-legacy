...
class RoleManager extends Base
{
    ...
    function __construct()
    {
        ...
        $this->domain_roles = new \Innomatic\Dataaccess\Nestedset\FullNestedSet($this->domainda, "domain_roles", "ID", "left", "right");
    }
    ...
    function HasPermission($Role, $Permission)
    {
        $Res = jf::SQL("
					...
					(SELECT left FROM domain_roles WHERE ID={$Role})
					AND
					(SELECT right FROM domain_roles WHERE ID={$Role})
    ...
    {
        if ($OnlyIDs) {
            $Res = jf::SQL("SELECT permissionid AS `ID` FROM domain_roles_permissions WHERE roleid={$Role} ORDER BY permissionid");
            if (is_array($Res)) {
                $out = array();
                foreach ($Res as $R)
                    $out[] = $R['ID'];
			...
			WHERE roleid={$Role} ORDER BY TP.permissionid");
    }
}
