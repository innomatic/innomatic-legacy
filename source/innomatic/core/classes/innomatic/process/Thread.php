...
class Thread
{
    ...
    public function register_callback_func($arglist, $methodname)
    {
        ...
        if (is_array($arglist) && count($arglist) > 1) {
            if ($arglist[1] == self::PHP_FORK_RETURN_METHOD)
                $this->_internal_ipc_array['_call_type'] = self::PHP_FORK_RETURN_METHOD;
            else
                $this->_internal_ipc_array['_call_type'] = self::PHP_FORK_VOID_METHOD;
        } else
            $this->_internal_ipc_array['_call_type'] = self::PHP_FORK_VOID_METHOD;
        ...
        switch ($this->_internal_ipc_array['_call_type']) {
            case self::PHP_FORK_VOID_METHOD :
                // notify the child so it can process the request
                $this->_sendSigUsr1();
                break;

            case self::PHP_FORK_RETURN_METHOD :
                // pseudo-semaphore +-+-+
                // +0+0+
                // +-+-+
