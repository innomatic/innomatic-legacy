...
class BinaryComponent extends \Innomatic\Application\ApplicationComponent
{
    public function __construct($rootda, $domainda, $appname, $name, $basedir)
    {
        ...
        if (!is_dir($this->container->getHome() . 'core/bin/')) {
            \Innomatic\Io\Filesystem\DirectoryUtils::mktree($this->container->getHome() . 'core/bin/', 0755);
        }
    }
    ...
    public function doInstallAction($params)
    {
        ...
        if (\Innomatic\Security\SecurityManager::isAboveBasePath($this->container->getHome() . 'core/bin/' . $params['name'], $this->container->getHome() . 'core/bin/')) {
        ...
        if ($dirname != '.') {
            \Innomatic\Io\Filesystem\DirectoryUtils::mktree($this->container->getHome() . 'core/bin/' . $params['name'], 0755);
        }
        // Copies the binary file.
        if (! copy($this->basedir . '/core/bin/' . $params['name'], $this->container->getHome() . 'core/bin/' . $params['name'])) {
        ...
        chmod($this->container->getHome() . 'core/bin/' . $params['name'], 0644);
        return true;
    }
    public function doUninstallAction($params)
    {
        ...
        if (! file_exists($this->container->getHome() . '/core/bin/' . $params['name'])) {
        ...
        if (! @unlink($this->container->getHome() . 'core/bin/' . $params['name'])) {
