...
class LibraryComponent extends \Innomatic\Application\ApplicationComponent
{
    ...
    public function doInstallAction($params)
    {
        ...
        if (strlen($params['file'])) {
            ...
            if (@copy($params['file'], $this->container->getHome() . 'core/lib/' . basename($params['file']))) {
                @chmod($this->container->getHome() . 'core/lib/' . basename($params['file']), 0644);
                $result = true;
            }
        } else
    ...
    public function doUninstallAction($params)
    {
        $result = false;
        if (strlen($params['file'])) {
            if (@unlink($this->container->getHome() . 'core/lib/' . basename($params['file']))) {
                $result = true;
            } else
                $this->mLog->logEvent('innomatic.phpcomponent.phpcomponent.douninstallaction', 'In application ' . $this->appname . ', component ' . $params['name'] . ': Unable to remove ' . $this->container->getHome() . 'core/lib/' . basename($params['file']), \Innomatic\Logging\Logger::ERROR);
        } else
